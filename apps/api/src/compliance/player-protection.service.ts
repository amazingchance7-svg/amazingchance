...
export class PlayerProtectionService {
  ...
  assertCanPurchase(
    ...
    return this.assertCanPurchaseWithClient(this.prisma, userId, at);
  }
  ...
  assertCanPurchaseInTransaction(
    ...
    return this.assertCanPurchaseWithClient(tx, userId, at);
  }
  ...
  startSelfExclusion(
    ...
    return this.prisma.$transaction(
      async (tx) => {
        ...
        const existing = await tx.selfExclusion.findFirst({
          where: {
            userId,
            startsAt: { lte: now },
        ...
        if (existing) {
          ...
          if (endsAt !== undefined && endsAt <= existing.endsAt) {
            return existing;
          }

          const extended = await tx.selfExclusion.create({
            data: {
              userId,
              startsAt: now,
              endsAt: endsAt ?? null,
              reason: reason?.trim() || 'SELF_EXCLUSION_EXTENDED',
            },
          });
          await tx.playerComplianceProfile.updateMany({
            ...
            data: {
              ...
              statusReason: 'SELF_EXCLUSION_ACTIVE',
            },
          });
          return extended;
        }
  ...
  private async assertCanPurchaseWithClient(
    ...
    const profile = await client.playerComplianceProfile.findUnique({
      where: { userId },
    });
    ...
    if (exclusion) {
      throw new ConflictException('Active self-exclusion blocks purchases');
    }

    const policies = await client.jurisdictionPolicy.findMany({
      where: {
    ...
    const age = this.ageAt(profile.dateOfBirth, at);
