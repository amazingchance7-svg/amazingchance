...
describe('Public snapshot integration', () => {
  ...
  beforeAll(async () => {
    ...
    publicSnapshot = new PublicSnapshotService(prisma);
  });
  ...
  async function createDrawWithSnapshot(finalizeSnapshot: boolean) {
  ...
  it('returns public metadata for a finalized snapshot', async () => {
    const scenario = await createDrawWithSnapshot(true);

    const result =
      await publicSnapshot.findFinalizedByDrawId(scenario.draw.id);
  ...
  it('serializes ticketCount as a JSON-safe string', async () => {
    ...
    expect(typeof result.ticketCount).toBe('string');
    expect(() => JSON.stringify(result)).not.toThrow();
  });

  it('returns not found for a building snapshot', async () => {
    const scenario = await createDrawWithSnapshot(false);

    await expect(
      publicSnapshot.findFinalizedByDrawId(scenario.draw.id),
    ).rejects.toThrow('Finalized ticket snapshot not found');
  });

  it('returns not found for an unknown draw', async () => {
    await expect(
      publicSnapshot.findFinalizedByDrawId(randomUUID()),
    ).rejects.toThrow('Finalized ticket snapshot not found');
  });
});
